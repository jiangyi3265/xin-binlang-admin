...
const DEFAULT_BRANDING={brand:'金榔记',brandEn:'JINLANGJI',brandMark:'榔',brandLogo:'',adminSubtitle:'总部运营中枢'}
...
function navigationGroups(){
  ...
  const groups=[['运营工作台',['dashboard','settings']],['活动资产',['pools','prizes','batches']],['履约网络',['stores','orders']],['安全与审计',['customers','audit','notifications']]]
...
async function settingsPage(main){
  ...
  const fallbackFlow=[
    {title:'购买活动产品',description:'购买带有“开码有奖”标识的金榔记产品'},
    {title:'获取数字兑换码',description:'打开包装，找到包装内的 6 位数字兑换码'},
    {title:'微信登录兑奖',description:'微信授权登录后输入兑换码，立即查看中奖结果'},
    {title:'到店出示凭证',description:'中奖后在有效期内选择门店，出示二维码完成核销'}
  ]
  ...
  main.innerHTML=pageHead('settings')+`<form class="panel settings-form" id="settings-form"><header class="panel-head"><div><h2>消费者端统一配置</h2><p>保存后，小程序重新进入或刷新时读取最新内容。</p></div>${badge(item.active?'active':'paused',item.active?'活动进行中':'活动已暂停')}</header><div class="settings-sections">
    <section class="settings-section"><div class="settings-section-head"><div><span>01</span><h3>品牌与活动基础</h3></div><p>品牌标识会同步到后台侧边栏、小程序登录页、首页和门店端。</p></div><div class="form-grid">
      ...
      ${field('方块文字标识','brandMark',item.brandMark||'榔','text','required maxlength="2"','未上传品牌 Logo 时显示，建议填写 1 个汉字。')}${field('后台副标题','adminSubtitle',item.adminSubtitle||'总部运营中枢','text','required maxlength="80"','显示在总部后台品牌名称下方。')}
      ${imageUploadField({label:'品牌 Logo',name:'brandLogo',value:item.brandLogo||'',title:'上传统一品牌 Logo',description:'建议 512 × 512 正方形 PNG，四周留安全边距。上传后替代方块文字标识；移除后自动恢复文字标识。'})}
...
function poolModal(item){
  openModal({title:item?'编辑奖池':'新建独立奖池',desc:'批次绑定奖池后，抽奖只会消耗该奖池内的奖品库存。',content:`<div class="form-grid">${field('奖池名称','name',item?.name||'','text','required maxlength="80"','名称可以重复，请通过产品档位和奖池说明区分不同规格。')}${field('产品档位','tier',item?.tier||'','text','required maxlength="80"')}${selectField('状态','status',item?.status||'active',[{value:'active',label:'启用'},{value:'disabled',label:'停用'}])}<div class="field span-2"><label for="f-desc">奖池说明</label><textarea class="textarea" id="f-desc" name="desc">${esc(item?.desc||'')}</textarea></div></div>`,onSubmit:async(form,close)=>{await api(`/admin/pools${item?'/'+item.id:''}`,{method:item?'PUT':'POST',body:Object.fromEntries(form)});close();toast('奖池已保存');loadPage()}})
}
...
function prizeModal(item,pools){
  ...
  openModal({title:item?'编辑奖品':'添加奖品',desc:'库存会在消费者中奖时原子扣减；库存为 0 时该奖项自动停止发放。',content:`<div class="form-grid">
    ...
    ${field('规格','spec',item?.spec||'')}${field('奖项等级','level',item?.level||'')}${selectField('类型','type',item?.type||'goods',[{value:'goods',label:'实物'},{value:'cash',label:'现金/红包'},{value:'coupon',label:'优惠券'}])}${field('参考价值（元）','valueYuan',yuanFromCents(item?.valueCents??0),'number','required min="0" max="100000" step="0.01" inputmode="decimal"','直接填写人民币金额，例如 30 表示 ¥30.00。')}
    ...
    ${imageUploadField({label:'中奖弹窗图片',name:'img',value:item?.img||'',title:'上传中奖后展示的奖品图片',description:'建议 1200 × 1200 正方形，奖品居中、背景干净；支持 PNG、JPG、WebP，不超过 5MB。',required:true})} ${selectField('状态','status',item?.status||'active',[{value:'active',label:'启用'},{value:'disabled',label:'停用'}])}</div>`,onSubmit:async(form,close)=>{if(imageUploading)throw new Error('图片仍在上传，请稍候');const values=Object.fromEntries(form);if(!values.img)throw new Error('请先上传奖品图片');values.valueCents=centsFromYuan(values.valueYuan,'参考价值');delete values.valueYuan;for(const key of ['stock','sent','lowStockThreshold','weight'])values[key]=Number(values[key]);await api(`/admin/prizes${item?'/'+item.id:''}`,{method:item?'PUT':'POST',body:values});close();toast('奖品已保存');loadPage()}})
...
function ordersTable(items,actions=true){return items.length?`<div class="table-wrap"><table class="table"><thead><tr><th>订单 / 兑换码</th><th>消费者</th><th>结果</th><th>状态</th><th>意向 / 核销门店</th><th>时间</th>${actions?'<th></th>':''}</tr></thead><tbody>${items.map(item=>`<tr><td><span class="cell-main code">${esc(item.code)}</span><span class="cell-sub">${esc(item.orderNo)}</span></td><td>${esc(item.userNick)}<span class="cell-sub">${esc(item.userPhone)}</span></td><td>${item.win?`<span class="cell-main">${esc(item.prizeName)}</span><span class="cell-sub money">${money(item.prizeValue)}</span>`:'未中奖'}</td><td>${badge(item.status)}</td><td>${esc(item.storeName||item.preferStoreName||'—')}<span class="cell-sub">${item.verifiedBy?`由 ${esc(item.verifiedBy)} 核销`:''}</span></td><td>${fmt(item.redeemAt)}<span class="cell-sub">${item.verifiedAt?`核销 ${fmt(item.verifiedAt)}`:`截止 ${fmt(item.expireAt)}`}</span></td>${actions?`<td><div class="actions">${item.status==='pending'&&!readOnly()?`<button class="btn btn-small btn-danger" data-freeze="${item.id}">冻结</button>`:''}${item.status==='frozen'&&!readOnly()?`<button class="btn btn-small" data-unfreeze="${item.id}">解冻</button>`:''}</div></td>`:''}</tr>`).join('')}</tbody></table></div>`:empty('没有符合条件的订单')}
...
const pageRenderers={dashboard:dashboardPage,settings:settingsPage,pools:poolsPage,prizes:prizesPage,batches:batchesPage,stores:storesPage,sales:salesPage,orders:ordersPage,customers:customersPage,audit:auditPage,notifications:notificationsPage}
