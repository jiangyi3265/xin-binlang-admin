...
const pages={
  ...
  prizes:{label:'奖品与库存',eyebrow:'INVENTORY',desc:'维护奖品内容、开奖资格、展示权重、库存和启停状态。',icon:'▦'},
...
function prizeModal(item,pools){
  let imageUploading=false
  openModal({title:item?'编辑奖品':'添加奖品',desc:'参与开奖的奖品会在中奖时扣减库存；仅展示的奖品只出现在展示卡片中，不参与开奖。',content:`<div class="form-grid">
    ...
    ${selectField('开奖资格','displayOnly',item?.displayOnly?'true':'false',[{value:'false',label:'参与开奖'},{value:'true',label:'仅展示（不参与开奖）'}])}${field('展示权重（相对值）','showcaseWeight',item?.showcaseWeight??1,'number','required min="0" max="100000" step="0.0001"','控制翻牌后其他展示卡片出现的相对机会，数值越大越容易展示；填 0 不出现在展示卡中，不影响实际中奖概率。每次最多展示 5 个奖项。')}
    <p class="hint span-2" data-display-only-hint>仅展示的奖品不会被抽中、发奖或扣减库存；展示卡片会按展示权重随机选取奖品，位置随机排列。候选超过 5 个时，较小权重的奖品会有机会不出现。只展示文字时无需上传图片。</p>
    ${field('当前库存','stock',item?.stock??0,'number','required min="0"')}${field('已发放数','sent',item?.sent??0,'number','required min="0"')}${field('低库存阈值','lowStockThreshold',item?.lowStockThreshold??10,'number','required min="0"')}${field('抽奖权重','weight',item?.weight??1,'number','required min="0.0001" max="100000" step="0.0001"','只影响参与开奖的奖品之间的分配；仅展示奖品不使用此数值。')}
    ${imageUploadField({label:'中奖弹窗图片',name:'img',value:item?.img||'',title:'上传中奖后展示的奖品图片',description:'建议 1200 × 1200 正方形，支持 PNG、JPG、WebP，不超过 5MB。现金红包和仅展示奖品可不上传图片；现金红包会使用默认红包图案。',required:false})} ${selectField('状态','status',item?.status||'active',[{value:'active',label:'启用'},{value:'disabled',label:'停用'}])}</div>`,onSubmit:async(form,close)=>{if(imageUploading)throw new Error('图片仍在上传，请稍候');const values=Object.fromEntries(form);values.displayOnly=values.displayOnly==='true';if(!values.displayOnly&&values.type!=='cash'&&!values.img)throw new Error('参与开奖的商品奖品请先上传图片');values.exchangeCents=values.type==='exchange'?centsFromYuan(values.exchangeYuan,'到店补款'):0;delete values.exchangeYuan;values.valueCents=centsFromYuan(values.valueYuan,'参考价值');delete values.valueYuan;for(const key of ['stock','sent','lowStockThreshold','weight','showcaseWeight'])values[key]=Number(values[key]);await api(`/admin/prizes${item?'/'+item.id:''}`,{method:item?'PUT':'POST',body:values});close();toast('奖品已保存');loadPage()}})
  ...
  const rewardType=q('[name=type]',overlay);const displayOnly=q('[name=displayOnly]',overlay);const syncRewardFields=()=>{const exchange=rewardType.value==='exchange';const showcaseOnly=displayOnly.value==='true';q('[data-exchange-fields]',overlay).hidden=!exchange;const amount=q('[name=exchangeYuan]',overlay);amount.disabled=!exchange;amount.required=exchange;q('[data-cash-hint]',overlay).hidden=rewardType.value!=='cash'||showcaseOnly;q('[data-display-only-hint]',overlay).hidden=!showcaseOnly;q('[name=weight]',overlay).readOnly=showcaseOnly};rewardType.addEventListener('change',syncRewardFields);displayOnly.addEventListener('change',syncRewardFields);syncRewardFields()
}

async function prizesPage(main){
  const [items,pools]=await Promise.all([api('/admin/prizes'),api('/admin/pools')]);main.innerHTML=pageHead('prizes',readOnly()?'':`<button class="btn btn-primary" id="add">＋ 添加奖品</button>`)+`<section class="panel"><div class="filters"><div class="field grow"><label>奖品配置</label><div class="hint">仅展示奖品不参与开奖；展示权重控制其他卡片的展示机会。参与开奖且低于库存阈值的奖品以橙色提示。</div></div></div><div class="table-wrap"><table class="table"><thead><tr><th>奖品</th><th>奖池</th><th>价值</th><th>库存 / 阈值</th><th>已发</th><th>抽奖 / 展示权重</th><th>状态 / 开奖资格</th><th></th></tr></thead><tbody>${items.map(item=>`<tr><td><div class="prize-cell"><div class="prize-thumb">${item.img?`<img src="${attr(item.img)}" alt="">`:'无图'}</div><div><span class="cell-main">${esc(item.name)}</span><span class="cell-sub">${esc(item.level)} · ${esc(item.spec)}${item.type==='exchange'?` · 补 ${money(item.exchangeAmount)} 换 1 袋`:item.type==='cash'?' · 微信现金红包':''}</span></div></div></td><td>${esc(item.poolName)}</td><td class="money">${money(item.value)}</td><td><strong style="color:${!item.displayOnly&&item.stock<=item.lowStockThreshold?'var(--orange)':'inherit'}">${item.stock}</strong> / ${item.lowStockThreshold}</td><td>${item.sent}</td><td><span class="cell-main">抽奖：${item.displayOnly?'不参与':esc(item.weight)}</span><span class="cell-sub">展示：${esc(item.showcaseWeight??1)}${Number(item.showcaseWeight??1)===0?'（不展示）':''}</span></td><td>${badge(item.status)}<span class="cell-sub">${item.displayOnly?'仅展示':'参与开奖'}</span></td><td><div class="actions">${readOnly()?'':`<button class="btn btn-small" data-edit="${item.id}">编辑</button>`}</div></td></tr>`).join('')}</tbody></table></div></section>`
